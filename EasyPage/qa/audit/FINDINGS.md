...
> - `qa/audit/walkthrough.mjs` —— 18 个场景（含 1 个 1024×820 窄视口场景、2 个纯悬停场景），每场景截图 + 注入式审计函数
> - `qa/audit/shots/*.png` —— 逐场景截图
> - `qa/audit/report.json` —— 机读结果（截断 / 零尺寸 / 越界 / 重叠 / 遮挡 / 区域溢出 / 大面积着色 / 高亮滞留 / 可滚动性）
>
> T123 追加见下方同名章节（用户报「蓝色框」）。
...
> 用户原话：「现在最致命的问题就是蓝色框，非常影响使用；」
>
> 定位方式：先做**排除性**取证，再逐点移动鼠标记录 `#ep-hover-box` 的命中目标与几何。
> 排除项（都被实测否掉）：壳层全量枚举蓝边框 → 0 项；`#ep-overlay-root` 无选中态下
> 全部 `display:none`（`checkVisibility` 均 false）；画布外框是灰色 `1px solid rgb(222,222,216)`；
> `[data-ep-editing]` 虚线框只在就地编辑期存在且提交即摘。
> **结论：是 hover 高亮框，不是选中框 / 手柄 / 画布外框 / focus ring。**

### T123-1 悬停页面空白区 → 整个文档被套上蓝框 + 蓝蒙层（P0）

复现（1440×900，`qa/fixtures/01-script-carousel.html`）：

| 步骤 | 修复前 `#ep-hover-box` | 修复后 |
|---|---|---|
| 悬停画布底部空白 | `display:block rect=[16,106,1406,322] bg=rgba(66,133,244,0.12)` | `display:none` |
| 鼠标移到顶栏「样式」 | `display:block rect=[16,106,1406,322]`（滞留） | `display:none` |
| 样式面板已打开 | `display:block rect=[16,106,1406,322]`（仍在） | `display:none` |

成因链（三条独立缺陷叠加，单独修任何一条都还有症状）：

1. 内容不足一屏时，悬停空白处 `pointermove` 的 `e.target` 是 `html` / `body`，
   于是 `toOverlayBox` 算出来的框 = **整个文档**（1406×322）。
2. `#ep-hover-box` 的填充是 `--ep-accent-soft: rgba(66,133,244,0.12)` —— 铺在按钮 / 标题上是「高亮」，
   铺在整页上就是**盖住全部内容的蓝蒙层**。叠加 2px 蓝描边，正是用户说的「蓝色框」。
3. `CanvasHost` 只监听了 iframe 内的 `pointermove`，**没有 `pointerleave`**（全仓 grep 无一处命中），
   所以指针移回顶栏调样式时高亮框一直不退 —— 而那恰恰是最需要看清画布的时刻。

修复（判定在 TS，表现交给 CSS，与既有 `data-open` / `data-danger` 同一套写法）：

- `geom.isDocumentRoot(el)`：`html` / `body` 在 hover 阶段直接不高亮。
  框恒等于整页，高亮它没有信息量；需要选根时走面包屑，功能不减。
- `geom.hoverFillAllowed(box, frameRect)` + `HOVER_FILL_MAX_RATIO = 0.25`：
  框面积占画布 > 1/4 的大容器**只描边不填充**，小元素保留填充以辨认嵌套层级。
  表现层一条：`#ep-hover-box[data-fill='false'] { background: none; }`
- `CanvasHost.onLeave` + `cd.documentElement.addEventListener('pointerleave')`：
  指针离开画布立即撤销高亮。
  ⚠️ `pointerleave` **不冒泡**，挂 `document` 收不到，必须挂 `documentElement`。

修复后行为（同一脚本实测，阈值 25%）：

| 落点 | 命中元素 | `#ep-hover-box` |
|---|---|---|
| 画布底部空白 | `html` / `body` | `display:none` |
| `<p>` 说明文字（占画布 5%） | `p` | `block` + `bg=rgba(66,133,244,0.12)`（描边 + 填充） |
| `.carousel` 容器（占画布 42%） | `div.carousel` | `block` + `bg=rgba(0,0,0,0)`（只描边） |
| 顶栏 / 画布外 | — | `display:none` |

回归：`tests/unit/geom.test.ts`（`hoverFillAllowed` 5 例 + `isDocumentRoot` 2 例）；
`tests/e2e/selection.spec.ts` → 「hover 高亮：文档根不套框 / 大容器只描边 / 离开画布即撤（T123）」。
...
| **检测项本身写着却不生效** | T123 加「大面积着色」检查时，正则写成 `/rgba?\(\s*[\d.]+.../`。**`AUDIT_FN` 整体是模板字符串**，单反斜杠会被 JS 当转义吞掉（`\s`→`s`、`\d`→`d`），正则静默失配、alpha 恒为 0 ⇒ 检测器**永远返回 0**。文件里既有的 `label()` 早就写成 `\\s+` 就是在避这个坑，我照抄时漏了。现已把 alpha 解析改写成**不含正则**的 `bgAlphaOf()`。教训：模板字符串里写正则，一律 `\\s` / `\\d`；或者干脆别用正则 |
| **检测器写完必须用「缺陷态」验一次灵敏度** | 上面那条如果只跑「修复后」的代码，会看到漂亮的 `0` 并以为检测器没问题（**假绿**）。做法：把修复点临时回滚（本仓库用 `sed` 改 `geom.ts` 的两个常量 + 给 `pointerleave` 套 `if (false)`）→ 重新 build → 跑走查，**确认两个场景各自报出 1 项** → 再还原。此外还发现「场景把缺陷自己擦掉了」这一类设计错误：最初 17 号场景是「悬停空白 → 再移到顶栏」，结果指针一移开就触发了要测的撤销逻辑，快照里缺陷已经不存在。**取快照的时刻必须是缺陷活着的那一刻** ⇒ 拆成 17（停住不动）/ 18（移出去）两个场景 |
| 单场景的 fixture 太「满」会让缺陷无法触发 | 默认 fixture `05-ai-landing.html` 内容正好铺满画布，没有空白区可悬停；就算命中 `html/body`，框也只占画布 ~25%，卡在阈值边缘。已给场景表加 `fixture` 覆盖字段，场景 17 用自带的短文档（`body{height:480px}`，把内容高度钉死成画布 80%），让缺陷**必然**复现 |
...
| 单测 | 183 / 183（T122 时 176，T123 新增 `hoverFillAllowed` 5 + `isDocumentRoot` 2） |
| e2e（`--workers=1` 串行） | 67 / 67（T122 时 66，T123 新增 hover 高亮回归 1） |
| 走查 18 场景 | 18 / 18，异常项 0（T122 时 16 场景；T123 新增 17/18 两个悬停场景 + `bigTint` / `staleHover` 两条检查） |
