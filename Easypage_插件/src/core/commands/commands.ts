 ...
 * 写回元素**内联标记**（`innerHTML`）。
 *
 * 为什么不能拿 `SetTextCommand` 凑合 —— 它是 `textContent` 语义：
 *
 *   就地改字时，元素里可能带着行内标记（`<p>结论：<strong>留存下滑</strong></p>`）。
 *   若把「提交后的 innerHTML」交给 `SetTextCommand`，它会把这段 HTML **当成纯文本**写进去，
 *   于是页面上直接显示出 `<strong>留存下滑</strong>` 这串源码，而原来的 `<strong>` 元素没了。
 *   用户只是改了几个字，行内格式连同标签一起被毁。
 *
 * `SetTextCommand` 保持原样不动：旧形态的 `InlineTextEditor` 收敛的就是纯文本，
 * 把它的语义改成 HTML 会让存量文本里的 `&` / `<` 被当成标记解析 —— 那是另一场回归。
 * 两个语义分开，各管一摊。
 */
export class SetHtmlCommand implements ICommand {
  readonly name = 'set-html';
  readonly prev: string;
  readonly next: string;
  constructor(private readonly target: Element, next: string, prev: string) {
    this.next = next; this.prev = prev;
  }
  execute(): void { this.target.innerHTML = this.next; }
  undo(): void { this.target.innerHTML = this.prev; }
}
