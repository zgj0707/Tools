...
export class SelfInteractionAdapter implements InteractionAdapter {
  ...
  showGuides(guides: Guide[]): void {
    ...
    // 吸附线的落位（R2）：Guide.position 来自 outerRect，坐标系是
    // 「iframe 内部坐标 + frameRect 偏移」的混合值；要落到屏幕上还得乘缩放比。
    // 缩放 100% 时 zoom = 1，与改造前逐像素一致。
    const z = zoomOf(this.frame);
    const toScreen = (v: number, base: number) => (v - base) * z + base;
    for (const g of guides) {
      ...
      if (g.orientation === 'v') {
        ...
        line.style.left = `${toScreen(g.position, frameRect.left) - origin.left}px`;
        line.style.top = '0';
      } else {
        line.style.height = '1px';
        line.style.width = '100%';
        line.style.left = '0';
        line.style.top = `${toScreen(g.position, frameRect.top) - origin.top}px`;
      }
